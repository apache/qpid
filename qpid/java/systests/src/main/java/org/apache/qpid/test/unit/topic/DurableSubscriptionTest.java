...
public class DurableSubscriptionTest extends QpidTestCase
{
    ...
    public void testResubscribeWithChangedSelectorNoClose() throws Exception
    {
        ...
        // Send 1 matching message and 1 non-matching message
        MessageProducer producer = session.createProducer(topic);
        TextMessage msg = session.createTextMessage("testResubscribeWithChangedSelectorNoClose1");
        msg.setBooleanProperty("Match", true);
        producer.send(msg);
        msg = session.createTextMessage("testResubscribeWithChangedSelectorNoClose2");
        msg.setBooleanProperty("Match", false);
        producer.send(msg);
        ...
        assertNotNull(rMsg);
        assertEquals("Content was wrong", 
                     "testResubscribeWithChangedSelectorNoClose2",
                     ((TextMessage) rMsg).getText());
