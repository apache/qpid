...
{
    private static final Logger _log = LoggerFactory.getLogger(IoSender.class);
    ...
    public IoSender(Socket socket, int bufferSize, long timeout)
    {
        ...
        try
        {
            _senderThread = Threading.getThreadFactory().newThread(this);                      
        }
        catch(Exception e)
        {
        ...
        _senderThread.setName(String.format("IoSender-%s", socket.getRemoteSocketAddress()));
        _senderThread.start();
    }
    ...
    public void run()
    {
        ...
        while (true)
        {
            ...
            {
                _log.error("Error in write thread", e);
                _exception = e;
                close();
                break;
            }
