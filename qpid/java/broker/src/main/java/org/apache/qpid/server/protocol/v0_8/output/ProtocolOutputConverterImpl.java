...
class ProtocolOutputConverterImpl implements ProtocolOutputConverter
{
    ...
    private ContentHeaderBody getContentHeaderBody(AMQMessage msg)
            throws AMQException
    {
        return msg.getContentHeaderBody();
    }


    private void writeMessageDelivery(AMQMessage msg, int channelId, AMQBody deliverBody)
            throws AMQException
    {
        writeMessageDelivery(msg, getContentHeaderBody(msg), channelId, deliverBody);
    }
    ...
    private AMQMessage convertToAMQMessage(QueueEntry entry)
    {
        ...
        {
            msg = (AMQMessage)entry.getMessage();
        }
        else
        {
            msg = getMessageConverter(entry.getMessage()).convert(entry.getMessage(),entry.getQueue().getVirtualHost());
        }
        return msg;
    }
    ...
    public void writeGetOk(QueueEntry entry, int channelId, long deliveryTag, int queueSize) throws AMQException
    {
        ...
        AMQBody deliver = createEncodedGetOkBody(msg, entry.isRedelivered(), deliveryTag, queueSize);
        writeMessageDelivery(msg, channelId, deliver);
    }
    ...
    private AMQBody createEncodedGetOkBody(AMQMessage message, boolean isRedelivered, long deliveryTag, int queueSize)
            throws AMQException
    {
        final AMQShortString exchangeName;
        final AMQShortString routingKey;
